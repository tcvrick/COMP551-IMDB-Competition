...
class MultiLabelTextProcessor(DataProcessor):
    ...
    def __init__(self, data_dir, fold_id, split_size=5000):
        self.data_dir = data_dir
        self.labels = None

        df = pd.read_pickle('df_train.pkl')
        df = df.reindex(columns=['review_id', 'text', 'sentiment'])
    ...
    def get_train_examples(self, data_dir):
        # data_df = pd.read_pickle('inputs/df_train.pkl')[5000:]
        # data_df = data_df.reindex(columns=['review_id', 'text', 'sentiment'])
        # data_df.text = data_df.text.apply(fixup)
        return self._create_examples(self.df_train, labels_available=True)

    def get_dev_examples(self, data_dir):
        # data_df = pd.read_pickle('inputs/df_train.pkl')[:5000]
        # data_df = data_df.reindex(columns=['review_id', 'text', 'sentiment'])
        # data_df.text = data_df.text.apply(fixup)
        return self._create_examples(self.df_val, labels_available=True)

    def get_test_examples(self, data_dir, data_file_name, size=-1):
        # data_df = pd.read_pickle('inputs/df_test.pkl')
        # data_df = data_df.reindex(columns=['review_id', 'text'])
        # data_df.text = data_df.text.apply(fixup)
        return self._create_examples(self.df_test, labels_available=False)
    ...
    @staticmethod
    def _create_examples(df, labels_available=True):
...
def accuracy_thresh(y_pred: Tensor, y_true: Tensor, thresh: float = 0.5,):
    """Compute accuracy when `y_pred` and `y_true` are the same size."""
    return torch.eq((y_pred.sigmoid() > thresh).squeeze(), y_true.byte()).float().mean()
